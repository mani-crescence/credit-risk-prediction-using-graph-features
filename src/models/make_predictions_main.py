...
def processing(classic_train_path, classic_test_path, classic_config_path, configurations, original_directory, path, alpha = None, discretization_type = None):
    ...
    classic_configurations = ast.literal_eval(classic_configurations)
    
    for _, value in classic_configurations.items():
         classic_config = value
         
    trainset = trainset[classic_config]
    testset = testset[classic_config]
    ...
    if alpha:
        ...
        with open(original_directory+ '/real/' + path + '/metrics_results_' +  alpha + ".txt",
                  'w') as file:
            file.write(str(results_real))

        os.makedirs(original_directory + '/percent/' + path, exist_ok=True)
        with open(original_directory+ '/percent/' + path + '/metrics_results_' +  alpha + ".txt",
                  'w') as file:
...
if __name__ == "__main__":
    ...
    if  discretization_type == 'None' and graph_type == 'None':
        ...
        processing(classic_train_path, classic_test_path, classic_config_path, configurations, directory, directory_, None)
