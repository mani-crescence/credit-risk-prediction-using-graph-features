...
def launch_build_engine_for_unsupervized_discretization(db_name):
    ...
    _path = 'data/raw/train/trainset_'+ db_name +'.csv' 
    _dir = 'engine/without_normalization/discretization/'
...
def launch_unsupervised_discretization(db_name):
    ...
    for label in ["train", "test"]:
        _path = "data/raw/"+ label + "/"+ label +"set_" + db_name + ".csv"
        
        commands.append("""make run_unsupervised_discretization_{0} DB_NAME={1} _PATH={2} NORMALIZATION_LABEL={3}  DATA_LABEL={4} """.
                        format(*[db_name.lower(), db_name.lower(), _path, "without_normalization", label]))
...
def launch_create_edges_for_complete_graph(db_name):
    ...
    _dir = 'graph/without_normalization/'
    ...
    train_path = "data/raw/train/trainset_" + db_name + ".csv"
    test_path = "data/raw/test/testset_" + db_name + ".csv"
...
def launch_relate_edges_for_complete_graph(db_name):
    ...
    directory = 'graph/without_normalization/'+ db_name + '/subsets/train/' 
    train_path = "data/raw/train/trainset_" + db_name + ".csv"
    _dir = 'graph/without_normalization/'
    ...
    directory = 'graph/without_normalization/'+ db_name + '/subsets/test/'
    test_path = "data/raw/test/testset_" + db_name + ".csv"
...
def launch_graph_modeling(db_name):
    ...
    for graph_type in standard_proposed_graphs:
        for discretization_type in discretization_types:
            train_path = "data/without_normalization/discretized/" + db_name.lower() + "/discretized_train_data_" + discretization_type.lower() + ".csv"
            test_path = "data/without_normalization/discretized/" + db_name.lower() + "/discretized_test_data_" + discretization_type.lower() + ".csv"
...
def launch_compute_descriptors(db_name):
    ...
    _dir = 'data/without_normalization/graph_features/'
    _graph_dir = 'graph/without_normalization/'
...
def launch_config_without_stepwise(db_name):
    ...
    classic_train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    save_dir = 'data/without_normalization/without_stepwise/configurations'
    ...
    for graph_type in state_of_art_graphs:
        ...
        new_descriptor_train_path = "data/without_normalization/graph_features/" + db_name.lower() + "/" + graph_type.lower() + "/new_features_train.csv"
    ...
    for graph_type in standard_proposed_graphs:    
        for discretization_type in discretization_types:
            new_descriptor_train_path = "data/without_normalization/graph_features/" + db_name.lower() + "/" + graph_type.lower()  +"/"+ discretization_type.lower()  + '/train/' + "new_features_0.1.csv"
            
            commands.append("""make run_make_configurations_{0}  SAVE_DIR={1} DISCRETIZATION_TYPE={2} GRAPH_TYPE={3} CLASSIC_TRAIN_PATH={4} NEW_DESCRIPTOR_TRAIN_PATH={5}""".
                            format(*[db_name.lower(), save_dir, discretization_type, graph_type, classic_train_path, new_descriptor_train_path]))
            
    for graph_type in proposed_complete_graph:
        new_descriptor_train_path = "data/without_normalization/graph_features/" + db_name.lower() + "/" + graph_type.lower() + "/train/new_features_0.1.csv"
...
def launch_config_with_stepwise(db_name):
    ...
    classic_train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    save_dir = 'data/without_normalization/with_stepwise/configurations'
...
def launch_stepwise_selection(db_name):
    train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    save_dir = 'data/without_normalization/with_stepwise/configurations/'+ db_name.lower() 
...
def launch_predict(db_name):
    save_dir = 'reports/without_normalization/without_stepwise/'+ db_name + '/metrics'
    classic_train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    classic_test_path = "data/raw/test/testset_" + db_name.lower() + ".csv"
    classic_config_path = "data/without_normalization/without_stepwise/configurations/"+ db_name + "/configuration_classic.txt" 
    
    commands = []
    
    for graph_type in state_of_art_graphs:
            train_path = 'data/without_normalization/graph_features/' + db_name.lower() + '/' + graph_type.lower() + '/new_features_train.csv'
            test_path = 'data/without_normalization/graph_features/' + db_name.lower() + '/' + graph_type.lower() + '/new_features_test.csv'
            config_path = "data/without_normalization/without_stepwise/configurations/" + db_name.lower()+"/configuration_" + graph_type.lower() + ".txt"
    ...
    for graph_type in standard_proposed_graphs:
        for disc_type in discretization_types:
            train_directory = 'data/without_normalization/graph_features/' + db_name.lower() + '/' + graph_type .lower() + "/" +  disc_type.lower() + '/train'
            test_directory = 'data/without_normalization/graph_features/' + db_name.lower() + '/' + graph_type .lower() + "/" + disc_type.lower() + '/test' 
            
            config_path = "data/without_normalization/without_stepwise/configurations/" + db_name.lower() + "/configuration_" + graph_type.lower() + "_" + disc_type.lower() + ".txt"
...
def launch_predict_classic(db_name):
    train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    test_path = "data/raw/test/testset_" + db_name.lower() + ".csv"
    config_path = "data/without_normalization/without_stepwise/configurations/" + db_name.lower() + "/configuration_classic.txt" 
    save_dir = 'reports/without_normalization/without_stepwise/' + db_name.lower() + '/metrics/'
...
def launch_predict_with_stepwise(db_name):
    save_dir = 'reports/without_normalization/with_stepwise/'+ db_name + '/metrics'
    classic_train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    classic_test_path = "data/raw/test/testset_" + db_name.lower() + ".csv" 
    ...
    for graph_type in standard_proposed_graphs:
        for disc_type in discretization_types:
            ...
            config_path = "data/without_normalization/with_stepwise/configurations/" + db_name.lower() + "/configuration_" + graph_type.lower() + "_" + disc_type.lower() + ".txt"
...
def launch_predict_classic_with_stepwise(db_name):
    train_path = "data/raw/train/trainset_" + db_name.lower() + ".csv"
    test_path = "data/raw/test/testset_" + db_name.lower() + ".csv"
    config_path = "data/without_normalization/with_stepwise/configurations/" + db_name + "/configuration_classic.txt" 
    save_dir = 'reports/without_normalization/with_stepwise/'+ db_name + '/metrics/'
...
def launch_print(db_name):
    ...
    directory = "reports/without_normalization/without_stepwise/" + db_name + "/metrics/"
...
def launch_print_with_stepwise(db_name):
    ...
    directory = "reports/without_normalization/with_stepwise/" + db_name + "/metrics/"
