...
def generate_result_without_stepwise():
    global_results = {}
    save_dir = "reports/without_normalization/without_stepwise/summary"

    for db in db_names:
        global_results[db] = {}
        for graph, discretization in zip(graphs, discretizations):
            if discretization is None and graph is not None:
                with open('reports/without_normalization/without_stepwise/' + db + '/metrics/real/' + graph + '/main_results_r.txt') as file:
                    result = ast.literal_eval(file.read())
            elif discretization is None and graph is None:
                with open('reports/without_normalization/without_stepwise/'+ db +'/metrics/classic/main_results_r.txt') as file:
                    result = ast.literal_eval(file.read())
            else:
                with open('reports/without_normalization/without_stepwise/'+ db +'/metrics/real/'+ graph + '/main_results_' + graph + '_'+ discretization +'_r.txt') as file:
                    result = ast.literal_eval(file.read())
...
def generate_result_with_stepwise():
    global_results = {}
    save_dir = "reports/without_normalization/with_stepwise/summary"

    for db in db_names:
        global_results[db] = {}
        for graph, discretization in zip(graphs, discretizations):
            if discretization is None and graph is not None:
                with open('reports/without_normalization/with_stepwise/' + db + '/metrics/real/' + graph + '/main_results_r.txt') as file:
                    result = ast.literal_eval(file.read())
            elif discretization is None and graph is None:
                with open('reports/without_normalization/with_stepwise/'+ db +'/metrics/classic/main_results_r.txt') as file:
                    result = ast.literal_eval(file.read())
            else:
                with open('reports/without_normalization/with_stepwise/'+ db +'/metrics/real/'+ graph + '/main_results_' + graph + '_'+ discretization +'_r.txt') as file:
                    result = ast.literal_eval(file.read())
            global_results[db].update(result)
