...
def main(target, db_name, alpha, graph_type, discretization_type = None):
    ...
    if discretization_type is not None:
        discretized_data_train  = pd.read_csv("outputs/"+db_name+"/data/discretized/graph/discretized_train_"+discretization_type +".csv", index_col=0, dtype='int')
        ...
        with open("outputs/"+db_name+"/graph/graph_"+ graph_type.lower() + '_' + discretization_type,"rb" ) as f:
            graph_data = pickle.load(f)

        build_graph_attributes(graph_data["graph"], graph_data["descriptors"], target, db_name, alpha, "train", graph_type, train_data, None, discretization_type)
...
if __name__ == "__main__":
    ...
    if len(args) > 4:
      ...
      main(target, db_name, alpha, graph_type, discretization_type)
    else:
